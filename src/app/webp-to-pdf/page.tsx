...
export default function WebpToPdfPage() {
  ...
  const convertToPdf = async () => {
    ...
    try {
      ...
      for (const file of files) {
        // Convert WebP to PNG/JPEG via canvas
        const img = new Image();
      ...
      pdf.setProducer('Toolverse WebP to PDF');
  ...
  return (
    <ToolLayout title="WebP to PDF" description="Convert WebP images to PDF format"
      ...
      <div onDrop={handleFileDrop} onDragOver={(e) => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)}
        className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all ${dragActive ? 'border-lime-500 bg-lime-500/5' : 'border-white/10 hover:border-white/20'}`}>
        <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-lime-500/20 to-green-500/20 flex items-center justify-center">
          <svg className="w-8 h-8 text-lime-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
        </div>
        <h3 className="text-lg font-semibold mb-2">Drop WebP images here</h3>
        <p className="text-gray-400 text-sm mb-4">Or click to browse</p>
        <label className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-lime-500 to-green-500 rounded-xl cursor-pointer hover:opacity-90 font-medium">
          Add Images<input type="file" accept="image/webp,image/*" multiple onChange={handleFileSelect} className="hidden" />
        </label>
      </div>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {files.map((file, i) => (
              <div key={i} className="flex items-center gap-3 bg-black/20 rounded-xl p-3 border border-white/5">
                <div className="w-8 h-8 rounded bg-lime-500/20 flex items-center justify-center text-sm">{i + 1}</div>
          ...
          {!result && (
            <div className="bg-black/20 rounded-xl p-4 border border-white/5">
              ...
              <div className="flex gap-2">{(['portrait', 'landscape'] as const).map(o => (
                <button key={o} onClick={() => setOrientation(o)} className={`flex-1 py-2 rounded-lg text-sm ${orientation === o ? 'bg-lime-500' : 'bg-white/5 text-gray-400'}`}>{o}</button>
              ))}</div>
            </div>
          )}
        </div>
      ...
      {files.length > 0 && !result && (
        <button onClick={convertToPdf} disabled={processing} className="mt-6 w-full py-4 bg-gradient-to-r from-lime-500 to-green-500 rounded-2xl font-semibold text-lg hover:opacity-90 disabled:opacity-50">
      ...
      {result && (
        <div className="mt-6 space-y-4">
          ...
          <a href={result.url} download="webp_to_pdf.pdf" className="block w-full py-4 bg-gradient-to-r from-emerald-500 to-teal-500 rounded-2xl font-semibold text-center hover:opacity-90">Download PDF</a>
