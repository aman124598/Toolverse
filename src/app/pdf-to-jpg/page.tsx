...
export default function PdfToJpgPage() {
  ...
  const downloadImage = async (url: string, page: number) => {
    const blob = await (await fetch(url)).blob();
    await Mobile.saveFile(blob, `${file?.name.replace('.pdf', '')}_page_${page}.jpg`);
  };

  const downloadAll = () => {
    images.forEach((img, _i) => {
      setTimeout(() => { void downloadImage(img.url, img.page); }, _i * 200);
    });
  };
      ...
      {!file ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-blue-500/10 to-indigo-500/10 rounded-xl p-5 border border-blue-500/20">
            ...
            <button onClick={() => { void Mobile.openUrl(URL.createObjectURL(file)); }} className="mt-4 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 rounded-lg text-sm text-blue-300 transition-colors">
              Open PDF in New Tab
            </button>
          </div>
        </div>
