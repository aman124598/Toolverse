...
export function MyStackDrawer() {
  ...
  useEffect(() => {
    if (pathname === '/') {
      const timeout = setTimeout(() => {
        if (!localStorage.getItem('toolverse_drawer_reminder_v2')) {
  ...
  const dismissReminder = (e?: React.MouseEvent) => {
    ...
    setShowReminder(false);
    localStorage.setItem('toolverse_drawer_reminder_v2', 'true');
  };
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div 
        className={`fixed top-0 right-0 bottom-0 w-full sm:w-96 bg-black/90 backdrop-blur-xl border-l border-white/10 z-[70] transform transition-transform duration-300 ease-out flex flex-col shadow-2xl shadow-purple-500/10 ${
          isStackOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
